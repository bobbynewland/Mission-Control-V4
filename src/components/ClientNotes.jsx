...
export default function ClientNotes({ clientId }) {
  ...
  const createNewNote = async (type = 'note') => {
    ...
    const newNote = {
      ...
      tags: [],
      starred: false,
      clientId: clientId, // Set clientId for the note
      created: timestamp,
      updated: timestamp
    };
  ...
  // Filter notes by tab, search, AND clientId
  const filteredNotes = Object.entries(notes)
    .filter(([_, note]) => {
      // Only show notes for this client
      if (note.clientId !== clientId) return false;
          ...
          <div className="flex gap-2">
            {TABS.map((tab) => {
              const count = tab.id === 'all'
                ? Object.values(notes).filter(n => n.clientId === clientId).length
                : Object.values(notes).filter(n => n.clientId === clientId && n.type === tab.id).length;
              return (
                <button
                  key={tab.id}
